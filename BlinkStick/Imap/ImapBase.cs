...
namespace Joshi.Utils.Imap
{
	...
	{
		protected static readonly ILog log = LogManager.GetLogger("IMAP Client");	
			...
			try 
			{
                ...
                if(sslEnabled)
                {
                    ...
                    catch (AuthenticationException e)
                    {
                        ...
                        log.Info("Authentication failed - closing the connection.");
                        m_oImapServ.Close();
		...
		protected ImapResponseEnum SendAndReceive (string command, ref ArrayList sResultArray)
		{
			...
			ImapResponseEnum eImapResponse = ImapResponseEnum.IMAP_SUCCESS_RESPONSE;
			if (sCommand.StartsWith ("IMAP002 LOGIN")) {
				string[] commandBits = sCommand.Split(' ');
				if (commandBits.Length >= 3)
				{
					log.DebugFormat("{0} {1} {2} **********", commandBits[0], commandBits[1], commandBits[2]);
				}
			} else {
				log.Debug (sCommand.TrimEnd (IMAP_COMMAND_EOL.ToCharArray ()));
			}
...
/// <returns></returns>
        ...
        public bool ValidateServerCertificate(object sender, X509Certificate certificate, X509Chain chain, SslPolicyErrors sslPolicyErrors)
        {
            ...
            log.ErrorFormat("Failed to validate Server Certificate. Error: {0}", sslPolicyErrors);
