			...
			ref int nUsedKBytes, ref int nTotalKBytes)
		{
			...
			try
			{
				...
				if (eImapResponse == ImapResponseEnum.IMAP_SUCCESS_RESPONSE)
				{
						...
						if (sLine.StartsWith(quotaPrefix) == true)
						{
								...
								nEnd > nStart) 
							{
								...
								if (sQuota.Length > 0) 
								{
									...
									{
										log.Error("Invalid Quota information :" + sQuota);
										break;
									}
								}
								else 
							...
							{
								log.Error("Invalid Quota IMAP Response : " + sLine);
								break;
							}
						}
					...
					if (bUnlimitedQuota)
						log.Debug("GETQUOTA quota=[unlimited].");
					else
					{
						log.Debug("GETQUOTA used=[" + nUsedKBytes.ToString() + 
		...
		void GetBodyStructure(string sMessageUID, XmlTextWriter oXmlWriter, bool bFetchBody)
		{
			...
			try
			{
				...
				if (eImapResponse == ImapResponseEnum.IMAP_SUCCESS_RESPONSE)
				{
					...
					{
						log.Info("Bodystructure is empty");
						return;
					}
			...
			string sPartPrefix, bool bFetchBody)
		{
			...
			if (sBodyStruct[0] == '(')
			{
				...
				if (ch != '(')
				{
					...
					{
						log.Error("Failed getting Message Id.");
						return false;
					}
					if (sTemp.Length > 0) 
					{
					...
					if (!ParseQuotedString( ref sBodyStruct, ref sTemp )) 
					{
						log.Error("Failed getting the Content Desc.");
						return false;
					}
					if (sTemp.Length > 0) 
					{
					...
					if (!ParseQuotedString( ref sBodyStruct, ref sTemp )) 
					{
						log.Error("Failed getting the Content Encoding.");
						return false;
					}
					...
					{
						log.Error("Failed getting the Content Size.");
						return false;
					}
					...
					if (sTemp.ToLower() == IMAP_MESSAGE_RFC822.ToLower()) //email attachment
					{
						...
						{
							log.Error("Failed getting the Message Envelope.");
							return false;
						}
						...
						{
							log.Error ("Failed getting Attached Message.");
							return false;
						}
						// No of Lines in the message
						if (!ParseString(ref sBodyStruct, ref sTemp )) 
						{
							log.Error("Failed getting the Content Lines.");
							return false;
						}
						if (sTemp.Length > 0) 
						{
				...
				else // MULTIPART
				{
					...
					do 
					{
							...
							bFetchBody == true)
						{
							...
							{
                                log.Error("Exception:Invalid Body Structure. Error:" + e.Message);
								return false;
							}
						}
				...
				if (sBodyStruct[0] == ' ') 
				{
					...
					{
						log.Error("Failed getting the Content Disp.");
						return false;
					}
					if (sTemp.Length > 0) 
					{
			...
			log.Error ("Invalid Body Structure");
			return false;
		}
/// <summary>
		...
		void GetBody(string sMessageUid, string sPartNumber, ref string sData)
		{
			...
			try
			{
				...
				if (eImapResponse == ImapResponseEnum.IMAP_SUCCESS_RESPONSE)
				{
					...
					if (!sLine.StartsWith(IMAP_UNTAGGED_RESPONSE_PREFIX))
					{
						log.Error ("InValid Message Response " + sLine + ".");
...
/// <returns></returns>
			...
			ref string sContentType)
		{
			...
			if (sType.Length < 1) 
			{
				...
				{
					log.Error("Failed getting Content-Type.");
					return false;
				}
			}
			sSubType = "";
			...
			{
				log.Error("Failed getting Content-Sub-Type.");
				return false;
			}
			...
			{
				log.Error("Failed getting Content-Type Parameters.");
				return false;
			}
			...
			ref string sDisp)
		{
			...
			{
				log.Error("Failed getting Content Disposition.");
				return false;
			}
			...
			if (!ParseParameters( ref sBodyStruct, asParam )) 
			{
				log.Error("Failed getting Content Disposition params.");
				return false;
			}
		...
		bool ParseQuotedString(ref string sBodyStruct, ref string sString)
		{
			...
			log.Error("InValid Body Structure " + sBodyStruct + ".");
			return false;
		}
		/// <summary>
...
/// <returns>true if success</returns>
		...
		bool ParseLanguage(ref string sBodyStruct, ref string sString)
		{
			...
			{ // One or no language
				...
				{
					log.Error("Failed getting Content Language.");
					return false;
				}
			}
...
/// <returns>true if success</returns>
			...
			ArrayList asParams)
		{
			...
			while (sBodyStruct[0] != ')') 
			{
				...
				{
					log.Error("Invalid Body Parameter Name.");
					return false;
				}
				...
				{
					log.Error("Invalid Body Parameter Value.");
					return false;
				}
				asParams.Add(sName);
				asParams.Add(sValue);
...
/// <returns>true if success</returns>
		bool ParseAddress(ref string sBodyStruct, ref string sString)
		{
			...
			{
				log.Error("Failed getting the Personal Name.");
				return false;
			}
			// At Domain List (Right now, don't know what to do with this)
			if (!ParseQuotedString( ref sBodyStruct, ref sTemp )) 
			{
				log.Error("Failed getting the Domain List.");
				return false;
			}
			// Email Id
			if (!ParseQuotedString( ref sBodyStruct, ref sEmailId )) 
			{
				log.Error("Failed getting the Email Id.");
				return false;
			}
			// Email Domain
			if (!ParseQuotedString( ref sBodyStruct, ref sEmailDomain )) 
			{
				log.Error("Failed getting the Email Domain.");
				return false;
			}
			...
			ArrayList asAttrs)
		{
			...
			if (!ParseQuotedString( ref sBodyStruct, ref sTemp )) 
			{
				log.Error("Invalid Message Envelope Date.");
				return false;
			}
			if (sTemp.Length > 0) 
			{
			...
			if (!ParseQuotedString( ref sBodyStruct, ref sTemp )) 
			{
				log.Error("Invalid Message Envelope Subject.");
				return false;
			}
			if (sTemp.Length > 0) 
			{
			...
			{
				log.Error("Invalid Message Envelope From.");
				return false;
			}
			if (sTemp.Length > 0) 
			{
			...
			if (!ParseAddressList(ref sBodyStruct, ref sTemp )) 
			{
				log.Error("Invalid Message Envelope Sender.");
				return false;
			}
			if (sTemp.Length > 0) 
			{
			...
			if (!ParseAddressList(ref sBodyStruct, ref sTemp )) 
			{
				log.Error("Invalid Message Envelope Reply-To.");
				return false;
			}
			if (sTemp.Length > 0) 
			{
			...
			if (!ParseAddressList(ref sBodyStruct, ref sTemp )) 
			{
				log.Error("Invalid Message Envelope To.");
				return false;
			}
			if (sTemp.Length > 0) 
			{
			...
			if (!ParseAddressList(ref sBodyStruct, ref sTemp )) 
			{
				log.Error("Invalid Message Envelope CC.");
				return false;
			}
			if (sTemp.Length > 0) 
			{
			...
			if (!ParseAddressList(ref sBodyStruct, ref sTemp )) 
			{
				log.Error("Invalid Message Envelope BCC.");
				return false;
			}
			if (sTemp.Length > 0) 
			{
			...
			if (!ParseQuotedString( ref sBodyStruct, ref sTemp )) 
			{
				log.Error("Invalid Message Envelope In-Reply-To.");
				return false;
			}
			if (sTemp.Length > 0) 
			{
			...
			if (!ParseQuotedString( ref sBodyStruct, ref sTemp )) 
			{
				log.Error("Invalid Message Envelope Message Id.");
				return false;
			}
			if (sTemp.Length > 0) 
			{
...
/// <returns></returns>
		long GetResponseSize(string sResponse)
		{
			...
			{
				log.Error("Size 0. No Message after this.");
				return 0L;
			}
